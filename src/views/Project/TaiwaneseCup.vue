<template lang="pug">
img.w-screem(:src='getImageUrl("Work-1-頁首大圖")')
.base.py-20
  .part-1.text-center.pb-10
  ...
  .Divide.mt-10.mb-2
  .flex.mt-8
    .part-2.flex.flex-col.justify-between(class='w-1/2')
      .pr-10(v-for='item in part2')
        button.border-2.border-blue.rounded-full.text-blue.m-auto.mb-8(v-if='item.isButton', class='hover:bg-blue hover:text-white') {{item.title}}
    ...
    .part-3.flex.flex-col.justify-between.border-l.border-gray.pl-10(class='w-1/2')
      .mb-8(v-for='item in part3')
...
.w-screem.bg-black
  .base.py-20.flex.flex-wrap.items-center.justify-between
    a.flex.items-center.justify-between.border-b.border-gray.pb-8.mb-8(v-for='item in hash', class='w-2/5', :href='`#${item.id}`')
      div
        p.text-3xl.text-white {{item.title}}
        p.text-blue {{item.subtitle}}
      img.w-5(:src='getImageUrl("Work-6")')

#Research.w-screem.bg-blue.text-center.py-20
...
.w-screem.bg-white.flex.flex-row-reverse.mt-20
  img(v-for='item in 4', :src='getImageUrl(`Work-${item}`)',class='w-1/4')
.base.bg-white.mt-10
  li.font-light(v-for='item in AnalysisLi') {{item}}
  .Divide.my-10
  .flex.flex-wrap.justify-around
    div.mb-5(v-for="item in AnalysisData1", class='w-5/12')
      img.w-full(:src='getImageUrl(`Work-1-競品-${item.image}`)')
      p.font-bold.text-2xl.text-blue.my-5 {{item.title}}
      p.font-bold.text-xl 優勢
      li.text-gray.font-light(v-for='good in item.good') {{good}}
      p.font-bold.text-xl.mt-5 劣勢
      li.text-gray.font-light(v-for='bad in item.bad') {{bad}}
  .Divide.my-10
  .flex.justify-around
    div(v-for='item in AnalysisData2', class='w-5/12')
...
#User.w-screem.bg-blue.text-center.mt-20.py-20
  ...
  .flex
    .pr-10(class='w-1/2')
      p.font-bold.leading-8 人員：
        span.font-light 主辦2人 / 裁判、選手、賽務各1人 / 觀眾4人
      p.font-bold.leading-8 年齡：
        span.font-light 25-42歲 性別：男5人、女4人
      p.font-bold.leading-8 方式：
        span.font-light 通話、視訊
      p.font-bold.leading-8 時間：
        span.font-light 15-20分鐘 / 人
      p.font-bold.leading-8 問題：
      p.font-light.leading-8 Q1：期望在網站上獲得的資訊及幫助? 
      p.font-light.leading-8 Q2：期待選手哪些資訊?個人社群連結? 
      p.font-light.leading-8 Q3：是否有過往賽事紀錄查詢需求? 
      p.font-light.leading-8 Q4：官網上還是外部表單完成賽事報名? 
      p.font-light.leading-8 Q5：官網上可直接購買賽事周邊商品的功能? 
      p.font-light.leading-8 Q6：休賽期能持續其他自潛賽事新聞更新?
    img(:src='getImageUrl("Work-1-訪談-人員")', class='w-1/2')
  .Divide.my-10

.w-screem.bg-white.text-center.pb-20
...
.w-screem.bg-blue.p-40.special
  p 123
      
#Persona.w-screem.bg-white.text-center.py-20
...
.base.bg-white.py-20.flex.justify-between
  div(v-for='item in PersonaData', class='w-1/4')
    ...
    p.font-bold.leading-8 名字：
      span.font-light {{item.name}}
    p.font-bold.leading-8 年齡：
      span.font-light {{item.age}}
    p.font-bold.leading-8 性別：
      span.font-light {{item.gender}}
    p.font-bold.leading-8 居住：
      span.font-light {{item.live}}
    p.font-bold.leading-8 職業：
      span.font-light {{item.job}}
    p.font-bold.leading-8 年資：
      span.font-light {{item.year}}
    p.font-bold.leading-8 收入：
      span.font-light {{item.income}}
    p.font-bold.leading-8.mt-6 動機：
      span.font-light {{item.motivation}}
    p.font-bold.leading-8 難題：
      span.font-light {{item.problem}}
    p.font-bold.leading-8 目標：
      span.font-light {{item.target}}


#Information.w-screem.bg-blue.text-center.py-20
...
.w-screem.bg-white.text-center.py-20
...
#Interaction.w-screem.bg-blue.text-center.py-20
...
img.w-screem(:src='getImageUrl("Work-1-線框稿")')
...
img.w-screem(:src='getImageUrl("Work-1-響應式介面")')
...
img.base(:src='getImageUrl("Work-1-可用性測試")')

#Visual.w-screem.bg-blue.text-center.py-20
...
.base.text-center
  ...
  p.font-bold.text-3xl.text-gray-bg DINPro
  .font-style.flex.justify-between.border-b.pb-1.mt-5.mx-auto.items-end
    p.text-4xl.font-black Taiwanese Cup
...
img.w-screem.mt-20(:src='getImageUrl("Work-1-頁尾大圖")')
...
export default {
  ...
  data: () => ({
    part2: [
      {
        title: "背景-Background",
        describe:
          "第二屆將擴大規模並成為定期舉辦之國際賽事，相關賽務資訊有線上統整必要性，以便選手及觀賽民眾能方便獲取，藉負責賽事主視覺機會進行官網設計規劃安排",
      },
      {
        title: "目標-Objectives",
        list: [
      ...
      },
      {
        title: "成果-Result",
        describe:
          "藉由競品分析與訪談研究釐清需求目標，透過資訊分類管理及功能流程架構的設定，完成滿足使用者需求、簡便易懂的操作體驗以及舒適友善的使用介面",
      },
      {
    ...
    hash: [
      ...
      { title: "使用者訪談", subtitle: "User Interviews", id: "User" },
      { title: "互動設計", subtitle: "interaction Design", id: "Interaction" },
      { title: "人物誌", subtitle: "Persona", id: "Persona" },
    ...
    AnalysisData1: [
      ...
      {
        title: "Asia Freediving Cup",
        image: "AFC",
        good: ["首頁完成最新賽事資訊", "中英韓版介紹貼心閱讀"],
        bad: ["排版過多文字及非當前必要性資訊", "功能與選項分類邏輯不清與重複"],
      },
      {
        title: "FINA",
        image: "FINA",
    ...
    AnalysisData2: [
      {
        ...
        content: [
          "作為台灣第一場自由潛水深度賽事，主辦方同時也致力於成為國際型賽事，在對外溝通與資訊傳達上需要一個正式且國際化的平台，在語言調性上需滿足中、英文雙語的服務。",
          "針對賽後成績的查詢與分類上，達成簡潔直覺的操作與篩選方式、協助選手於國際上增加曝光機會、完成站內周邊商品銷售目標功能，皆是台灣盃深度賽官方網站的重點任務。",
        ],
        list: [
          "中、英文同步的網站調性設定",
          "簡潔直覺的成績查詢與分類操作",
      ...
      },
      {
        ...
        content: [
          "固定每年三月底比賽期間較有賽務資訊的更新及流量，其他時間維持賽事回顧的更新及周邊商品銷售外，其他時間偏向靜態的資訊展示。設法就多媒體資訊、服務結合互動設計做為網站流量的延續。",
        ],
        list: [
          "延伸課程、服務與商品規劃",
          "獨家多媒體影音增加點閱流量",
    ...
    PersonaData: [
      {
        ...
        year: "7年",
        income: "NT60,000",
        motivation: "查詢選手資訊、隔日申報深度",
        problem: "社群更新頻繁難取得資訊",
        target: "登錄選手資訊、每日深度申報",
        img: "Work-1-人物誌-阿傑",
      },
      {
        character: "觀眾",
        ...
        year: "3年",
        income: "NT45,000",
        motivation: "找回會員密碼、購買周邊商品",
        problem: "連結到外部賣場步驟繁瑣",
        target: "一站式電商便利、分類易懂",
        img: "Work-1-人物誌-Janet",
      },
      {
        character: "選手",
        name: "Patty",
        ...
        year: "1年",
        income: "NT75,000",
        motivation: "完成報名、查詢往年天氣狀況",
        problem: "外部表單核對繁瑣，天氣難追查",
        target: "會員制度報名、新增歷年天氣",
